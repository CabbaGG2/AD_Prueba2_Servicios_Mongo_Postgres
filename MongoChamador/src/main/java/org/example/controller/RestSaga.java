...
public class RestSaga {
    public static final String MAPPING = "/mongo/saga";
    ...
    public ResponseEntity<List<Saga>> getByTitulo(@PathVariable String titulo){
        List<Saga> sagas = sagaService.findByTitulo(titulo).orElse(null);
    ...
    public ResponseEntity<Saga> create(@RequestBody Saga saga){
        Saga sagaGuardada = sagaService.crearActualizarSaga(saga);
